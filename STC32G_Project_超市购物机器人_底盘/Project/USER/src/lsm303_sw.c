...
void iic_lsm303_sendack(bit ackornack)
{
    // 产生一个IIC应答信号
...
void iic_lsm303_sendbyte(unsigned char dat)
{
    // 发送一个字节数据
    unsigned char i;
    for(i=0;i<8;i++)
...
unsigned char iic_lsm303_recvbyte()
{
    // 读取一个字节数据
...
void iic_lsm303_singlewrite()
{
    iic_lsm303_start();
    iic_lsm303_sendbyte(SlaveAddr);
    iic_lsm303_sendbyte(RegAddr);
    iic_lsm303_sendbyte(RegData);
    iic_lsm303_stop();
}

void iic_lsm303_singleread(unsigned char SlaveAddr, unsigned char RegAddr, unsigned char RegData)
{
    ...
    iic_lsm303_sendbyte(RegAddr);
    iic_lsm303_start();
    iic_lsm303_sendbyte(SlaveAddr + 1);
    RegData = iic_lsm303_recvbyte();
    iic_lsm303_sendack(1);
    iic_lsm303_stop();
    return RegData;
}

void iic_lsm303_multiread(unsigned char SlaveAddr, unsigned char STAddr)
{
    // 连续读取多个字节数据
    unsigned char i;
    iic_lsm303_start();
    iic_lsm303_sendbyte(SlaveAddr);
    iic_lsm303_sendbyte(STAddr);
    iic_lsm303_start();
    iic_lsm303_sendbyte(SlaveAddr + 1);
    for(i=0;i<6;i++)
    {
        BUF[i] = iic_lsm303_recvbyte();
        if (i==5)
        {
            iic_lsm303_sendack(1);
        }
...
void lsm303_acc_init(void) 
{
    ...
    // 写入寄存器
    iic_lsm303_singlewrite(LSM303_ACC_ADDRESS, LSM303_CTRL_REG1_A, CTRL_REG1);
    // iic_lsm303_write_reg(LSM303_ACC_ADDRESS, LSM303_CTRL_REG2_A, CTRL_REG2);
    // iic_lsm303_write_reg(LSM303_ACC_ADDRESS, LSM303_CTRL_REG3_A, CTRL_REG3);
    // iic_lsm303_write_reg(LSM303_ACC_ADDRESS, LSM303_CTRL_REG4_A, CTRL_REG4);
    // iic_lsm303_write_reg(LSM303_ACC_ADDRESS, LSM303_CTRL_REG5_A, CTRL_REG5);
    // iic_lsm303_write_reg(LSM303_ACC_ADDRESS, LSM303_CTRL_REG6_A, CTRL_REG6);
}
...
void lsm303_mag_init(void) {
    ...
    CRA_REG_M = 0x9C; 
    iic_lsm303_write_reg(LSM303_MAG_ADDRESS, LSM303_CRA_REG_M, CRA_REG_M);

    // 配置CRB_REG_M，增益配置为±1.3Gauss
    CRB_REG_M = 0x20; // 0010 0000
    // iic_lsm303_write_reg(LSM303_MAG_ADDRESS, LSM303_CRB_REG_M, CRB_REG_M);

    // 配置MR_REG_M，设置为连续转换模式
    MR_REG_M = 0x00; // 0000 0000
    iic_lsm303_singlewrite(LSM303_MAG_ADDRESS, LSM303_MR_REG_M, MR_REG_M);
    // iic_lsm303_write_reg(LSM303_MAG_ADDRESS, LSM303_MR_REG_M, MR_REG_M);
}
...
float lsm303_read_acc(char axis) 
{
    ...
    // 读取加速度计的数据
    acc_data[0] = iic_lsm303_read_reg(LSM303_ACC_ADDRESS, OUT_X_L_A);
    acc_data[1] = iic_lsm303_read_reg(LSM303_ACC_ADDRESS, OUT_X_H_A);
...
float lsm303_read_mag(char axis) 
{
    ...
    iic_lsm303_read_reg(LSM303_MAG_ADDRESS, addr_high, &mag_data_hi);
    iic_lsm303_read_reg(LSM303_MAG_ADDRESS, addr_low, &mag_data_lo);

    // 计算磁场值，由于磁场传感器的数据排列与加速度计不同，需要先读高位
    mag_value = ((float)(mag_data_hi << 8 | mag_data_lo)) / 1100.0f;
...
double lsm303_read_mag_angle()
{
    ...
    iic_lsm303_read_reg_bytes(LSM303_MAG_ADDRESS, LSM303_OUT_X_L_M, mag_data, 6);
