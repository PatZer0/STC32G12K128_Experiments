...
float LSM303_Read_Acc(char axis) 
{
    ...
    // 读取加速度计的数据
    LSM303_MasterReceiveMultipleBytesfromSlave(LSM303_ACC_ADDRESS, addr_low, acc_data, 2);
...
float LSM303_Read_Mag(char axis) 
{
    ...
    LSM303_MasterRecieveOneBytefromSlave(LSM303_MAG_ADDRESS, addr_high, mag_data[0]);
    LSM303_MasterRecieveOneBytefromSlave(LSM303_MAG_ADDRESS, addr_low, mag_data[1]);
...
double LSM303_Read_Mag_Angle()
{
    ...
    LSM303_MasterReceiveMultipleBytesfromSlave(LSM303_MAG_ADDRESS, LSM303_OUT_X_L_M, mag_data, 6);
